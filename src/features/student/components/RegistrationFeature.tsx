...
export function RegistrationFeature({ session }: { session: UserSession }) {
    ...
    useEffect(() => {
        if (yearOptionsData.length > 0 && year === 0) {
            let activeYear = Number(yearOptionsData[0].year_name);
            let activeSemester = yearOptionsData[0].semesters?.[0]?.semester_number || 1;
            
            for (const y of yearOptionsData) {
                const activeSem = y.semesters?.find((s: any) => s.is_active);
                if (activeSem) {
                    activeYear = Number(y.year_name);
                    activeSemester = activeSem.semester_number;
                    break;
                }
            }
    ...
    const isInitLoading = year === 0 || advisorQuery.isLoading || cartQuery.isLoading || registeredQuery.isLoading || academicYearsQuery.isLoading;
